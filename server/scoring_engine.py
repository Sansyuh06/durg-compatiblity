...
from server.kaggle_data import (
    DRUG_PROPERTIES, DRUGBANK_PK, CYP_INHIBITION,
    OFF_TARGET_BINDING, BBBP_DATA, TOX21_DATA, FAERS_SIGNALS,
)
...
def _cyp_modifier(patient: PatientProfile, drug_id: str) -> dict[str, Any]:
    ...
    for cyp in substrates:
        phenotype = getattr(patient.genetics, cyp, None) if hasattr(patient.genetics, cyp) else None
    ...
    return {
        "modifiers": modifiers,
        "composite_ke_multiplier": worst_multiplier * liver_mult,
        "liver_multiplier": liver_mult,
        "data_sources": ["CPIC Guidelines (cpicpgx.org)", "AASLD Liver Classification"],
    }
...
def pipeline_pk(patient: PatientProfile, drug_id: str, dose_mg: float = 0,
    ...
    accumulation = 1 / (1 - math.exp(-ke_adjusted * tau)) if ke_adjusted * tau < 20 else 1
...
def pipeline_off_target(patient: PatientProfile, drug_id: str) -> dict[str, Any]:
    """Pipeline 5: Off-target binding from ChEMBL + patient-specific penalties."""
    ...
    for target_name, data in targets.items():
        ...
        penalties.append({
            ...
            "source": f"ChEMBL bioactivity — {drug_id.upper()} vs {target_name}",
        })
    ...
    return {
        ...
        "data_sources": ["ChEMBL Bioactivity Database", "FAERS Population Signals"],
    }
...
def pipeline_admet(drug_id: str) -> dict[str, Any]:
    ...
    if not props:
        return {"score": 50, "confidence": "LOW", "data_sources": ["Estimated"]}
    ...
    score = 50
    if ro5_pass: score += 20
    if bbb_prob > 0.7: score += 15
    elif bbb_prob > 0.5: score += 8
    if psa < 90: score += 10
    if mw < 400: score += 5
    ...
    return {
        ...
        "lipinski": {
            ...
            "violations": violations, "pass": ro5_pass,
        },
        "bbb": {
            "probability": bbb_prob,
            "penetrant": bbb_prob > 0.5,
            "source": bbbp.get("source", "Estimated from PSA"),
        },
        "psa": psa,
        "rotatable_bonds": rot,
        "solubility_mg_ml": props.get("solubility_mg_ml"),
        "confidence": "HIGH",
        "data_sources": ["BBBP/MoleculeNet", "ChEMBL Molecular Properties", "PubChem"],
    }
...
def pipeline_composite(patient: PatientProfile, drug_id: str,
    ...
    genomic_score = 100 if abs(ke_mult - 1.0) < 0.1 else max(20, 100 - abs(1 - ke_mult) * 120)
    ...
    if (drug_id == "vpa"
        ...
        if ar.get("risk") == "HIGH":
            flags.append("WARNING: Androgen receptor binding — PCOS risk for young females")
